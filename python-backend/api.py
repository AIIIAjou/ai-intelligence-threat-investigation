...
def _get_agent_by_name(name: str):
    ...
    agents = {
        management_agent.name: management_agent,
        create_variations_agent.name: create_variations_agent,
        sns_search_agent.name: sns_search_agent,
        web_search_agent.name: web_search_agent,
        image_process_agent.name: image_process_agent,
    }
    return agents.get(name, management_agent)
...
def _build_agents_list() -> List[Dict[str, Any]]:
    ...
    return [
        make_agent_dict(management_agent),
        make_agent_dict(create_variations_agent),
        make_agent_dict(sns_search_agent),
        make_agent_dict(web_search_agent),
        make_agent_dict(image_process_agent),
    ]
...
async def chat_endpoint(req: ChatRequest):
    ...
    if is_new:
        ...
        current_agent_name = management_agent.name
        state: Dict[str, Any] = {
            "input_items": [],
            "context": ctx,
    ...
    try:
        ...
        refusal = "Sorry, I can only assist with intelligence gathering, OSINT investigations, and threat analysis topics."
    ...
    for item in result.new_items:
        ...
        # Handle handoff output and agent switching
            ...
            # If the tool is generate_intelligence_report, send a special message so the UI can render the report.
            if tool_name == "generate_intelligence_report":
                messages.append(
                    MessageResponse(
                        content="DISPLAY_INTELLIGENCE_REPORT",
                        agent=item.agent.name,
                    )
                )
